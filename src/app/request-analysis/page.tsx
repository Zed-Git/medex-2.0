...
// [NOVO vs Zlatni Standard: endpoint za kreiranje zahtjeva + slanje prvog e-maila]
...
// Ovo rješava problem kada NEXT_PUBLIC_BASE_URL nije podešen u .env.local
function getBaseUrl() {
  // Ako je definisan u env → koristimo njega (produkcija / vercel)
  if (process.env.NEXT_PUBLIC_BASE_URL) {
    return process.env.NEXT_PUBLIC_BASE_URL;
  }
  // Inače pretpostavljamo lokalni razvoj
...
export async function POST(req: NextRequest) {
  try {
    ...
    // --- [SUB-BLOCK: SEND PROCESSING EMAIL] ---
    // [KLJUČNO: NE šaljemo pdfUrl → send-email šalje "processing" email pacijentu + adminu]
    ...
    try {
      ...
      if (!emailResponse.ok) {
        ...
        console.log('[request-analysis] send-email triggered successfully.');
      }
    } catch (emailError) {
      console.error('[request-analysis] Email error (fetch failed):', emailError);
      // Ne rušimo glavni tok – zahtjev je već upisan u bazu
    }
